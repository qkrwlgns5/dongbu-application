...
`GET /api/school/participants`는 학교 세션·현재 대회·신청 기간·학교 활성 상태를 검증합니다. 쿼리로 다른 대회를 지정할 수 없으며, 활성 학교·종목·종별의 참가 응답만 학교명·종별·팀 수로 반환합니다. 계정 정보, 다른 학교의 수정 이력·미신청 상태, 관리자 데이터는 반환하지 않습니다. 기존 저장 기록을 읽으므로 추가 마이그레이션은 필요하지 않습니다.
